...
void Environment::start() {
    ...
    while (args != "exit") {
        ...
        if(verbose ==2 || verbose == 3)
         cout <<fs.getWorkingDirectory().getAbsolutePath() + ">" + whichCommand(args)->toString() + " " +args.substr(args.find_first_of(' ')+1) << endl;
...
BaseCommand *Environment::whichCommand(string arg) {
    ...
    if (command == "pwd") {
        ...
        BaseCommand *error;
        if(argsCommand == "error")
            error = new ErrorCommand("");
        else
            error = new ErrorCommand(argsCommand);
        return error;
    }
}